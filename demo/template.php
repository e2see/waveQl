...
<body>
    ...
    <!-- Presets werden dynamisch aus JSON geladen -->
        ...
        <div class="dashboard-row">
            <div class="dashboard-card">
                <fieldset class="compact-fieldset">
                    ...
                    $normalFields = [];
                    $dateFieldGroups = []; // für jedes Basis-Datum: [baseField => [list of auto fields]]

                    // Baue $dateFieldGroups aus den $dateFields (in test.php definiert)
                    ...
                    // Normale Felder: alle Filterfelder, die nicht in einem $dateFieldGroup als Auto oder Base vorkommen
                    $allDateRelated = array_merge($dateFields, ...array_values($dateFieldGroups));
                    $normalFields = array_diff($filterFields, $allDateRelated);

                    // Ausgabe der normalen Felder
                    ...
                    <?php foreach ($dateFieldGroups as $baseField => $autoFields): ?>
                        <?php
                            ...
                            // Platzhalter für das Eingabefeld
                            $placeholder = 'z.B. 5><9 oder >3';
                            // Datalist für Monatszahlen? Optional
                            $datalistId = '';
                            $datalistOptions = [];
                        ?>
                        <div class="date-field-group" data-datefield="<?= $baseField ?>">
                            ...
                            <div style="display: flex; gap: 0.5rem; align-items: center;">
                                ...
                                <input type="text" class="date-input" name="<?= $currentField ?>" value="<?= htmlspecialchars($inputValue) ?>" placeholder="<?= $placeholder ?>" list="<?= $datalistId ?>" style="flex: 1;">
                            </div>
                            <?php if (!empty($datalistOptions)): ?>
                            <datalist id="<?= $datalistId ?>">
                                <?php foreach ($datalistOptions as $opt): ?>
