...
<body>
    ...
    if ($allowInitSQL) {
        echo '<p><a href="?initSQL=1">Click here to reset / initialise the database</a></p>';
    } else {
        echo '<p><span class="disabled-element" style="cursor: default;">Reset / initialise database (disabled in config)</span></p>';
    }
    ?>
    ...
    <?php
    if ($initMessage) {
        $msgClass = strpos($initMessage, 'successfully') !== false ? 'success' : 'error';
        echo '<div class="message ' . $msgClass . '">' . $initMessage . '</div>';
    }

    if ($errorMsg) {
        echo '<div class="message error">' . htmlspecialchars($errorMsg) . '</div>';
    }

    if ($modeError) {
        echo '<div class="message error">' . htmlspecialchars($modeError) . '</div>';
    }


    if (!$modeError) {
        if (!$allowRead && $allowWrite) {
            echo '<div class="message warning">Read mode is disabled. Only write operations allowed.</div>';
        } elseif ($allowRead && !$allowWrite) {
            echo '<div class="message warning">Write mode is disabled. Only read operations allowed.</div>';
        }
    }
    ?>
